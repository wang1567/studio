...
export default function ProfilePage() {
  const { user: authUser, session, profile, isLoadingAuth, likedDogs, logout } = usePawsConnect();
  const router = useRouter();
  const [formattedJoinDate, setFormattedJoinDate] = useState<string | null>(null);

  useEffect(() => {
    if (!isLoadingAuth && !session) { // Redirect if not loading and no session
      router.replace('/auth');
    }
  ...
  if (isLoadingAuth || (!session && !profile)) { // Check session instead of just authUser
    return (
  ...
  if (!session || !profile) { // Ensure profile is also loaded along with session
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)]">
