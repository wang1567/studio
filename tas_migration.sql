-- TAS 認養中心資料表創建和資料插入 SQL
-- 請在 Supabase 儀表板的 SQL 編輯器中執行此檔案
...
-- 建立唯一約束以防止重複資料
CREATE UNIQUE INDEX IF NOT EXISTS idx_tas_adoption_centers_unique 
    ON public.tas_adoption_centers(area, organization_name, address);
...
-- 確認資料是否成功插入
SELECT COUNT(*) as total_centers FROM public.tas_adoption_centers;
SELECT area, COUNT(*) as center_count FROM public.tas_adoption_centers GROUP BY area ORDER BY area;